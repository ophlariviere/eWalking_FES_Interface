...
class MyInterface(GenericInterface):
    ...
    def get_kinematics_from_markers(
        ...
        **kwargs,
        ...
        if get_markers_data:
            marker_data = self.get_marker_set_data(marker_set_name)
        else:
            marker_data = None
        ...
        if not self.offline_data:
            ...
            kinematics = np.random.rand(nb_dof, 1), np.random.rand(nb_dof, 1)
        else:
            marker_set_idx = [i for i, m in enumerate(self.marker_sets) if m.name == marker_set_name][0]
            kinematics = self.marker_sets[marker_set_idx].get_kinematics(
                model_path, method, custom_func=custom_func, **kwargs
            )
            # Retourner les marqueurs s'ils sont demandés
        if get_markers_data:
