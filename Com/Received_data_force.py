...
class DataReceiver(QObject):
    ...
    def stimulation_process(self, force_data_oneframe):
        if len(self.fyr_buffer) > 40:
            info_feet = {
                "right": self.detect_phase_force(self.fyr_buffer, self.fzr_buffer, self.fzl_buffer, 1),
                "left": self.detect_phase_force(self.fyl_buffer, self.fzl_buffer, self.fzr_buffer, 2),
            }
    ...
    def detect_phase_force(self, data_force_ap, data_force_v, data_force_opp, foot_num):
        ...
        if (
            force_vert_last < 0.05 * subject_mass
            or (force_ap_previous < force_ap_last and force_ap_last > -0.01 * subject_mass)
    ...
    def check_cycle(self, data_force_v, received_data):
        ...
        if (
            force_v_last > 0.1 * self.visualization_widget.mass
            and force_v_previous < force_v_last
            and self.markers_data_cycle
        ):
