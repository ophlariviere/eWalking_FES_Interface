...
class DataReceiver(QObject):
                ...
                try:
                    ...
                    if received_data["footswitch_data"]:  # Ensure we have valid data before proceeding
                        ...
                        if self.visualization_widget.dolookneedsendstim:
                            ...
                            if emg_num:
                                info_feet = {
                                    "right": self.heel_off_detection(footswitch_data[emg_num['Right Heel'] - 1] ** 2,
                                                                     footswitch_data[emg_num['Right Toe'] - 1] ** 2, 1),
                                    "left": self.heel_off_detection(footswitch_data[emg_num['Left Heel'] - 1] ** 2,
    ...
    def heel_off_detection(self, data_foot_switch_heel, data_foot_switch_toe, foot_num):
        ...
        if (np.nanmean(np.abs(data_foot_switch_heel)) > 300 and np.nanmean(np.abs(data_foot_switch_toe)) > 360000 and
