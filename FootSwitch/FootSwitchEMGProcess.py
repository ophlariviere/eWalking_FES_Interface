...
class FootSwitchEMGProcessor:
    ...
    def heel_off_detection(self, data_foot_switch_heel, data_foot_switch_toe, foot_num):
        ...
        if (np.nanmean(np.abs(data_foot_switch_heel)) > 800 and (np.nanmean(np.abs(data_foot_switch_toe)) < 100) and
        ...
        if (np.nanmean(np.abs(data_foot_switch_heel)) > 800 and np.nanmean(np.abs(data_foot_switch_toe)) > 800 and
