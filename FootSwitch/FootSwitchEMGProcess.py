...
class FootSwitchEMGProcessor:
    ...
    def heel_off_detection(self, data_foot_switch_heel, data_foot_switch_toe, foot_num):
        print(
            f"{round(np.nanmean(np.abs(data_foot_switch_heel)))} and "
            f"{round(np.nanmean(np.abs(data_foot_switch_toe)))}"
        )
        if (
            np.nanmean(np.abs(data_foot_switch_heel)) > 800
            and (np.nanmean(np.abs(data_foot_switch_toe)) < 100)
        ...
        if (
            np.nanmean(np.abs(data_foot_switch_heel)) > 800
            and np.nanmean(np.abs(data_foot_switch_toe)) > 800
